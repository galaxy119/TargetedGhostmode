...
namespace TargetedGhostmode
{
	...
	public class Patch
	{
		...
		static bool Prefix(PlayerPositionManager __instance)
		{
			...
			foreach (GameObject player in players) posData.Add(new PlayerPositionData(player));
			...
			foreach (GameObject gameObject in players)
			{
				...
				if (smGhostMode && gameObject != __instance.gameObject && component.curClass >= 0)
					for (int i = 0; i < posData.Count; i++)
					{
						...
						if (smGhostMode && component2.smGhostMode && component2.curClass >= 0 &&
						    component2.curClass != 2 &&
